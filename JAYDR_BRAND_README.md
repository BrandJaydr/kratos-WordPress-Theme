...
- **Full PHP 8.5 & WordPress 7.0 Compatibility Overhaul**:
    - **Action**: Completed site-wide modernization of core logic and third-party integrations to ensure stable operation on PHP 8.5 and WordPress 7.0 (Final Release: May 20, 2026).
...
### Security Hardening
- **Red Ranger Security Initiative**:
    - **Action**: Neutralized site-wide high-impact vulnerabilities, including SQL Injection, CSRF, and Stored XSS, as part of a comprehensive security hardening cycle.
    - **SQL Injection**: Implemented `$wpdb->prepare()` for all dynamic queries in `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php` to neutralize potential injection vectors in post and comment queries.
    - **CSRF Protection**: Added nonce verification (`check_admin_referer` and `wp_nonce_field`) to all admin-facing forms and AJAX handlers, specifically in `inc/live2d/live2d.php` and `inc/QPlayer/option.php`.
    - **Contextual Escaping**: Enforced strict output escaping (`esc_attr`, `esc_url`, `esc_html`) across all templates, neutralizing Stored XSS in Bilibili comment metadata.
...
| :--- | :--- | :--- | :--- |
| 2026-05-12 | Vulnerability | Site-wide SQL Injection in `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php` | Fixed |
| 2026-05-05 | Vulnerability | CSRF in `inc/live2d/live2d.php` and `inc/QPlayer/option.php` | Fixed |
...
- **Fix**: Restored the English-localized `preg_split` implementation in `inc/myfunction.php`. Verified that it correctly handles English words, Chinese characters, and HTML tags.
- **Prevention**: Use automated tests (`test_wordcount.php`) to verify string manipulation functions during major compatibility overhauls.

### Site-wide SQL Injection (2026-05-12)
- **Vulnerability**: High-impact SQL injection vulnerabilities in `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php` due to unsanitized dynamic queries.
- **Fix**: Neutralized by wrapping all dynamic database queries in `$wpdb->prepare()`, ensuring proper parameterization of post IDs and comment data.
- **Prevention**: Enforce a strict policy of using `$wpdb->prepare()` for all dynamic SQL operations; use static analysis tools to detect raw query string concatenation.

### CSRF in Admin Pages (2026-05-05)
- **Vulnerability**: Cross-Site Request Forgery (CSRF) vulnerabilities in theme settings pages (Live2D and QPlayer) allowed unauthorized modification of configuration files and potential arbitrary file writes.
- **Fix**: Implemented WordPress nonce verification using `check_admin_referer` in form processing logic and `wp_nonce_field` in admin templates.
- **Prevention**: Ensure all state-changing actions in the WordPress admin dashboard are protected by nonces.
