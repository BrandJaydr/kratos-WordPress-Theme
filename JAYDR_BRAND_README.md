...
### Security Hardening
- **Red Ranger Security Hardening**:
    - **Neutralized SQL Injection**:
        - **Action**: Implemented `$wpdb->prepare()` for all dynamic database queries in `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php`.
    - **Neutralized Cross-Site Scripting (XSS)**:
        - **Action**: Applied strict input sanitization using `sanitize_text_field()` and `esc_url_raw()` for Bilibili comment metadata in `inc/myfunction.php`.
    - **Neutralized Cross-Site Request Forgery (CSRF)**:
        - **Action**: Added nonce verification using `check_admin_referer()` and `wp_nonce_field()` to all administrative forms in `inc/live2d/live2d.php`.
...
| :--- | :--- | :--- | :--- |
| 2026-04-28 | Vulnerability | SQL Injection, CSRF, and Unsanitized Input in core theme functions | Fixed |
...
### SQL Injection and CSRF Vulnerabilities (2026-04-28)
- **Error**: Detected missing SQL injection protection in `most_hot_posts()`, `most_comm_posts()`, and several shortcode/widget queries. Administrative forms in `live2d.php` also lacked CSRF protection.
- **Fix**: Implemented `$wpdb->prepare()` for all dynamic queries and added nonce verification to administrative forms.
- **Prevention**: Always use `$wpdb->prepare()` for database interactions and enforce nonce verification for all state-changing administrative actions.
...
- **Fix**: Restored the English-localized `preg_split` implementation and ensured correct HTML/entity decoding.
