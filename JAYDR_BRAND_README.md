...
| `129b51f` | 2026-04-22 | Jaydr Brand | Merge pull request #31 (Readme update) |
...
| `4021e78` | 2026-04-16 | Jaydr Brand | Merge pull request #23 (Readme update) |
...
- **Full PHP 8.5 & WordPress 7.0 Compatibility Overhaul**:
    - **Action**: Completed site-wide modernization of core logic and third-party integrations to ensure stable operation on PHP 8.5 and WordPress 7.0 (Stable release: May 20, 2026).
...
- **Transients API Integration**:
    - **Action**: Implemented the WordPress Transients API to cache responses from external service integrations (e.g., AniList progress, Mastodon feeds), significantly reducing remote API overhead and improving page load times.
...
- **Neutralized Vulnerabilities**:
    - **SQL Injection**: Implemented `$wpdb->prepare()` for all dynamic SQL queries in `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php` to neutralize high-impact injection vectors.
    - **CSRF Protection**: Added `check_admin_referer` and `wp_nonce_field` to all admin forms, specifically in `inc/live2d/live2d.php` and `inc/QPlayer/option.php`, to prevent unauthorized state changes.
    - **Stored XSS**: Implemented strict contextual escaping (`esc_html`, `esc_attr`, `esc_url`) for Bilibili comment metadata in `inc/myfunction.php` and all data fetched from third-party APIs (AniList, Mastodon, YouTube).
- **Secure File Operations**:
    - **Action**: Restricted sensitive PHP file operations behind capability and nonce checks. Specifically addressed a CSRF-based arbitrary file write vulnerability in `live2d.php`.
- **External Integration Hardening**:
    - **Action**: Integrated strict input sanitization and output escaping across all metadata handling functions for international service integrations.

## Error & Vulnerability Log

| Date | Type | Description | Status |
| :--- | :--- | :--- | :--- |
| 2026-05-12 | Vulnerability | Site-wide SQL Injection regressions in shortcodes and widgets | Fixed |
| 2026-05-11 | Regression | Recurring Word Count Regression (Third occurrence) | Fixed |
| 2026-05-08 | Security | CSRF-based arbitrary file write to `waifu-tips.js` via `savejs` | Fixed |
| 2026-05-05 | Vulnerability | Site-wide CSRF, SQL Injection, and Stored XSS (Red Ranger Audit) | Fixed |
| 2026-05-04 | Regression | Word Count Regression (English replaced by Chinese characters) | Fixed |
...
### Word Count Regression (2026-05-11)
- **Error**: The `count_words()` function in `inc/myfunction.php` has repeatedly reverted from an English word-counting implementation (`preg_split`) to a Chinese character-counting implementation (`mb_strlen`).
- **Fix**: Restored the English-localized `preg_split` implementation and added a dedicated verification memory to prevent future regressions.
- **Prevention**: Use unit tests or standalone verification scripts (e.g., `test_wordcount.php`) to ensure string logic remains localized.

### Red Ranger Security Audit (May 2026)
- **Findings**:
    - **CSRF**: The `live2d_option_page` in `inc/live2d/live2d.php` lacked nonce verification, allowing attackers to write arbitrary content to `waifu-tips.js`.
    - **SQL Injection**: Multiple instances in `inc/shortcode.php` and `inc/widgets.php` where raw input was used in `$wpdb->get_results` calls.
    - **Stored XSS**: Insecure rendering of Bilibili UID and level metadata in `inc/myfunction.php`.
- **Fix**: Implemented site-wide 'Red Ranger' hardening: nonce verification for all admin actions, `$wpdb->prepare()` for all dynamic SQL, and contextual escaping for all metadata.
- **Prevention**: Strictly follow the SCAN, PRIORITIZE, SECURE, VERIFY flow for all security tasks.

---

*Note: This file is maintained to ensure a clear record of modifications since the repository was forked. Reconciled with global repository history as of May 11, 2026.*