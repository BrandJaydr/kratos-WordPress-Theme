...
- **International Services Refactor**:
    - **Action**: Comprehensive refactoring of the service integration logic to decouple Chinese-specific dependencies and enable seamless switching between global alternatives like Mastodon and AniList (Commit `aebe761`).
...
- **Full Site-wide English Localization**:
    - **Action**: Completed comprehensive English localization across the entire theme, including PHP templates, JavaScript logic, and UI strings, ensuring a fully translated experience for international users (Commit `c167b7e`).
...
- **Fix**: Restored the English-localized `preg_split` implementation to correctly count words based on whitespace.
