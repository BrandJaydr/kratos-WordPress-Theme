...
- **'Red Ranger' Security Initiative**:
    - **Action**: Comprehensive neutralization of site-wide vulnerabilities including CSRF, SQL Injection, and Stored XSS.
    - **SQL Injection Prevention**: Implemented `$wpdb->prepare()` for all dynamic SQL queries across `inc/myfunction.php`, `inc/shortcode.php`, and `inc/widgets.php`.
    - **CSRF Protection**: Integrated `check_admin_referer()` and WordPress nonces for all frontend and backend forms, specifically targeting the Live2D settings in `inc/live2d/live2d.php`.
    - **XSS Mitigation**: Applied contextual escaping (`esc_attr()`, `esc_url()`, `esc_html()`) for all user-generated content and third-party metadata, including Bilibili integration fields in `inc/myfunction.php`.
...
| :--- | :--- | :--- | :--- |
| 2026-05-06 | Regression | Recurring English word count regression in `inc/myfunction.php` | Fixed |
| 2026-05-05 | Vulnerability | Site-wide CSRF, SQL Injection, and Stored XSS vulnerabilities | Fixed |
...
- **Error**: Multiple regressions where `count_words()` in `inc/myfunction.php` was reverted to a Chinese character-counting implementation using `mb_strlen`, breaking English word counting.
- **Fix**: Restored the English-localized `preg_split` implementation in commits `2a1b2b3`, `0401a95`, and `f899314`.
- **Prevention**: Use automated tests to verify string manipulation functions during major compatibility overhauls and implement a dedicated unit test for word counting.

### Site-wide Security Hardening (2026-05-05)
- **Error**: Detected multiple high-impact vulnerabilities including SQL Injection in widgets and shortcodes, CSRF in Live2D settings, and Stored XSS in Bilibili metadata.
- **Fix**: Commit `50fa884` (Red Ranger Initiative) neutralized these vulnerabilities by implementing `$wpdb->prepare()`, `check_admin_referer()`, and strict contextual escaping site-wide.
- **Prevention**: Enforce the use of WordPress security APIs (Nonces, Prepare, Escaping) for all new features and perform periodic security audits of the entire codebase.
