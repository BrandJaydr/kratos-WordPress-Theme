...
<div id="comments" class="comments-area">
    ...
    $args=array(
        ...
        'comment_notes_before' => '<p class="comment-notes">Nickname and UID are optional, but email is required (you will receive an email after someone replies to your message).<br>tips: Enter UID to quickly get your nickname and avatar.</p>',
        'comment_field'=>'<div class="comment form-group has-feedback"><div class="input-group"><textarea class="form-control" id="comment" placeholder="Looking forward to your wonderful comments~ φ(>ω<*) " name="comment" rows="5" aria-required="true" required  onkeydown="if(event.ctrlKey){if(event.keyCode==13){document.getElementById(\'submit\').click();return false}};"></textarea></div><div class="OwO"></div></div>',
    );
    comment_form($args); ?>
</div>