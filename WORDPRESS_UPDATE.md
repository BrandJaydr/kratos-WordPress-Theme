...
### April 2026 Update
- **PHP 8.5 Deprecation Handling:** Implemented conditional execution of `curl_close()` only for PHP versions earlier than 8.0. This change has been applied site-wide, including:
    - `inc/myfunction.php`
    - `inc/QPlayer/option.php`
    - `pages/bilibili/bilibiliAnime.php`
    - `pages/bilibililive/BilibiliLive.php`
    In PHP 8.0+, `CurlHandle` objects are automatically cleaned up, and `curl_close()` is a no-op that is deprecated in PHP 8.5.
