...
function timeago($ptime){
    ...
    $interval = array(
        12*30*24*60*60 => ' years ago'.' ('.date('M j',$ptime).')',
        30*24*60*60 => ' months ago'.' ('.date('M j',$ptime).')',
        7*24*60*60 => ' weeks ago'.' ('.date('M j',$ptime).')',
        24*60*60 => ' days ago'.' ('.date('M j',$ptime).')',
        60*60 =>' hours ago'.' ('.date('M j',$ptime).')',
        60 =>' minutes ago'.' ('.date('M j',$ptime).')',
        1 =>' seconds ago'.' ('.date('M j',$ptime).')',
    );
...
class kratos_widget_ad extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       => 'Advertisement',
            'description'=> 'Kratos theme feature component - Advertisement'
        );
    ...
    function form($instance){
        ...
        @$imgurl = esc_attr($instance['imgurl']); ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('aurl'); ?>">Link:
            ...
            <p>
                <label for="<?php echo $this->get_field_id('imgurl'); ?>">Image:
...
class kratos_widget_about extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'description'=> 'Kratos theme feature component - Personal Profile'
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
        ...
        <div class="textwidget">
            <div class="widget-admin text-center">
                <p>
                    <a href="<?php echo admin_url('/post-new.php'); ?>"><i class="fa fa-pencil"></i>Write Post</a>
                    <a class="widget-admin-center" href="<?php echo admin_url('/post-new.php?post_type=page'); ?>"><i class="fa fa-plus"></i>New Page</a>
                    <a href="<?php echo admin_url('/edit-comments.php'); ?>"><i class="fa fa-comments"></i>View Comments</a>
                </p>
                <p>
                    <a href="<?php echo admin_url('/options-general.php'); ?>"><i class="fa fa-cogs"></i>Site Settings</a>
                    <a class="widget-admin-center" href="<?php echo admin_url('/themes.php?page=kratos'); ?>"><i class="fa fa-cog"></i>Theme Settings</a>
                    <a href="<?php echo wp_logout_url($redirect); ?>"><i class="fa fa-sign-out"></i>Logout</a>
                </p>
            </div>
        </div>
        ...
        <div class="textwidget">
            <div class="widget-admin text-center">
                <p>
                    <a href="<?php echo admin_url('/profile.php'); ?>"><i class="fa fa-pencil"></i>Profile</a>
                    <a href="<?php echo admin_url('/'); ?>"><i class="fa fa-dashboard"></i>Dashboard </a>
                    <a href="<?php echo wp_logout_url($redirect); ?>"><i class="fa fa-sign-out"></i> Logout</a>
                </p>
            </div>
        </div>
        ...
        <div class="photo-wrapper clearfix">
            <div class="photo-wrapper-tip text-center">
                <a href="<?php echo wp_login_url($redirect); ?>" rel="nofollow"><img title="Click avatar to login" class="about-photo" src="<?php if(!empty($imgurl)) echo $imgurl; else echo bloginfo('template_url')."/static/images/photo.jpg"; ?>" alt=""/></a>
            </div>
        </div>
        <div class="textwidget">
    ...
    function form($instance){
        ...
        <p>
            <label for="<?php echo $this->get_field_id('imgurl'); ?>">Avatar URL:
                <input class="widefat" id="<?php echo $this->get_field_id('imgurl'); ?>" name="<?php echo $this->get_field_name('imgurl'); ?>" type="text" value="<?php echo $imgurl; ?>" />
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('profile'); ?>">Intro Content:
        ...
        <p>
            <label for="<?php echo $this->get_field_id('bkimgurl'); ?>">Card Background:
...
class kratos_widget_tags extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            'name'       => 'Tag Cloud',
            'description'=>'Kratos theme feature component - Tag Cloud'
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args, $instance){
    ...
    function form($instance){
        global $wpdb;
        $instance = wp_parse_args((array) $instance,array('title'=>'Tag Cloud','number'=>'20','orderby'=>'count','order'=>'RAND'));
        $title =  esc_attr($instance['title']);
        $number = intval($instance['number']);
        $orderby =  esc_attr($instance['orderby']);
        $order =  esc_attr($instance['order']); ?>
        <p>
            <label for='<?php echo $this->get_field_id("title"); ?>'>Title:<input type='text' class='widefat' name='<?php echo $this->get_field_name("title"); ?>' id='<?php echo $this->get_field_id("title"); ?>' value="<?php echo $title; ?>"/></label>
        </p>
        <p>
            <label for='<?php echo $this->get_field_id("number"); ?>'>Count:<input type='text' name='<?php echo $this->get_field_name("number"); ?>' id='<?php echo $this->get_field_id("number"); ?>' value="<?php echo $number; ?>"/></label>
        </p>
        <p>
            <label for='<?php echo $this->get_field_id("orderby"); ?>'>Type:
...
class kratos_widget_posts extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'name'       =>'Post Tab',
            'description'=>'Kratos theme feature component - Post Tab'
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
        ...
        <aside class="widget widget_kratos_poststab">
            <ul id="tabul" class="nav nav-tabs nav-justified visible-lg">
                <li class="active"><span href="#newest" data-toggle="tab">Recent Posts</span></li>
                <li><span href="#hot" data-toggle="tab">Hot Posts</span></li>
                <li><span href="#rand" data-toggle="tab">Random Post</span></li>
            </ul>
            <ul id="tabul" class="nav nav-tabs nav-justified visible-md">
                <li class="active"><span href="#newest" data-toggle="tab">Recent</span></li>
                <li><span href="#hot" data-toggle="tab">Hot</span></li>
                <li><span href="#rand" data-toggle="tab">Random</span></li>
            </ul>
    ...
    function form($instance){
        ...
        $number = intval($instance['number']); ?>
        <p>
            <label for='<?php echo $this->get_field_id("number"); ?>'>Items to display:<input type='text' name='<?php echo $this->get_field_name("number"); ?>' id='<?php echo $this->get_field_id("number"); ?>' value="<?php echo $number; ?>"/></label>
        </p>
        <input type="hidden" id="<?php echo $this->get_field_id('submit'); ?>" name="<?php echo $this->get_field_name('submit'); ?>" value="1" /><?php
    }
}
class kratos_widget_comments extends WP_Widget {
    function __construct() {
        $widget_ops = array(
            ...
            'description'=>'Kratos theme feature component - Recent Comments'
        );
        parent::__construct(false,false,$widget_ops);
    }
    function widget($args,$instance){
...
//                echo '<pre>';
    ...
    public function form($instance){
        $title = !empty($instance['title'])?$instance['title']:'Recent Comments';
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>">Display count:
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id('show_admin'); ?>">Do not show administrator (UID 1) comments</label>
        </p><?php
    }
}
