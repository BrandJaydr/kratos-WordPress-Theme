...
function kratos_banner(){
    if(!$output = get_option('kratos_banners')){
        ...
        if($kratos_banner_on){
            for($i=1; $i<6; $i++){
                $k_banner = kratos_option("kratos_banner{$i}")?kratos_option("kratos_banner{$i}"):"";
                $k_banner_url = kratos_option("kratos_banner_url{$i}")?kratos_option("kratos_banner_url{$i}"):"";
                if($k_banner){
                    $banners[] = $k_banner;
                    $banners_url[] = $k_banner_url;
                }
            }
            $count = count($banners);
