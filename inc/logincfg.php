...
function kratos_show_extra_register_fields(){ ?>
    ...
    <p>
        <label for="repeat_password">重复Password<br/>
...
function kratos_check_extra_register_fields($login,$email,$errors){
    if($_POST['nickname']=='') $errors->add('no_nickname',"<strong>Error</strong>：Nickname一栏不能为空。");
    if($_POST['password']!==$_POST['repeat_password']&&kratos_option('mail_reg')) $errors->add('passwords_not_matched',"<strong>Error</strong>：Passwords do not match。");
    if(strlen($_POST['password'])<8&&kratos_option('mail_reg')) $errors->add('password_too_short',"<strong>Error</strong>：Password长度必须大于8位。");
    if($_POST['are_you_human']!=$_POST['num1']+$_POST['num2']) $errors->add('not_human',"<strong>Error</strong>：验证码Error，try again。");
}
...
function limit_login_notify_email($user){
    ...
    $message = sprintf("Failed login attempts：%d ，IP：%s"
                  ."\r\n\r\n",$count,$ip);
    if ($user != '') {
        $message .= sprintf("Last attempted username：%s"
                    ."\r\n\r\n",$user);
    }
    $message .= sprintf('此 IP has been blocked，Lockout duration：%s',$when);
...
function limit_login_error_msg(){
    ...
    if($when>60){
        $when = ceil($when/60);
        $msg .= sprintf('Please  %d  hours后重试。',$when);
    } else {
        $msg .= sprintf('Please  %d  minutes后重试。',$when);
    }
    return $msg;
}
function limit_login_retries_remaining_msg(){
    ...
    return sprintf('<strong>Error</strong>：账号或Password有误，You still have <strong>%d</strong> attempt(s) left。',$remaining);
}
