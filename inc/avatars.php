...
class kratos_local_avatars{
    ...
    <table class="form-table">
        <tr>
            <th><label for="kratos-local-avatar">Upload Avatar</label></th>
            ...
            <td><?php
                if(kratos_option('lo_ava')||current_user_can('upload_files')){ ?>
                    ...
                    if(empty($profileuser->kratos_local_avatar)) echo '<span class="description">No local avatar set, please click "Browse" to upload a local avatar</span>';
                    else echo '
                            <input type="checkbox" name="kratos-local-avatar-erase" value="1" /> Remove local avatar<br />
                            <span class="description">If you need to modify the local avatar, Please re-upload a new avatar. If you need to remove the local avatar, Please select the above“Remove local avatar”checkbox and update your profile. <br/>Remove local avatar, will be restored to using Gravatar Avatar.</span>';
                }else{
                    if(empty($profileuser->kratos_local_avatar)) echo '<span class="description">No local avatar set, please set one at Gravatar.com.</span>';
                    else echo '<span class="description">You do not have permission to upload local avatars. Contact the administrator to change it.</span>';
                } ?>
            </td>
        </tr>
    </table>
    ...
    public function edit_user_profile_update($user_id){
        if(!empty($_FILES['kratos-local-avatar']['name'])){
            ...
            if(strstr($_FILES['kratos-local-avatar']['name'],'.php')) wp_die('.php cannot appear in the filename!');
            ...
            if(empty($avatar['file'])){
                switch($avatar['error']){
                    case 'Sorry, this file type is not permitted for security reasons.':
                        wp_die('Please upload files in jpg, gif, png, bmp, or tif format!');break;
                    default: wp_die('<strong>Upload AvatarThe following error occurred during the process: </strong> '.esc_attr($avatar['error']));
                }
                return;
            }
